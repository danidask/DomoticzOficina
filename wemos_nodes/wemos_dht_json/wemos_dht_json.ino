...
#include <DHT.h>
#include "credentials.h"  //Store ssid and password here
...
void read_sensor() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (isnan(humidity) || isnan(temperature)) {
      ...
      str_temperature[0] = '\0'; //null
      str_humidity[0] = '\0'; //null
      return;
    }
...
void setup(void)
{
  ...
  server.on("/API", [](){
    ...
    char response[70];
    char str_status[6];
    if (str_temperature[0] == '\0' || str_humidity[0] == '\0'){
      strcpy (str_status, "ERROR");
    }
    else {
      strcpy (str_status, "OK");   
    }
    snprintf(response, 75, "{\"status\": \"%s\",\"temperature\": \"%s\",\"humidity\": \"%s\"}", str_status, str_temperature, str_humidity);
